...
_d2.default.parcoords = function (config) {
  var __ = {
    data: [],
    highlighted: [],
    dimensions: {},
  ...
  if (config && config.dimensionTitles) {
    ...
    _d2.default.entries(config.dimensionTitles).forEach(function (d) {
  ...
  var pc = function pc(selection) {
    selection = pc.selection = _d2.default.select(selection);
  ...
  var events = _d2.default.dispatch.apply(this, ['render', 'resize', 'highlight', 'brush', 'brushend', 'brushstart', 'axesreorder'].concat(_d2.default.keys(__))),
      w = function w() {
    ...
    debug: false
      ...
      dragging = {},
      line = _d2.default.svg.line(),
      axis = _d2.default.svg.axis().orient('left').ticks(5),
      g,
  ...
  var side_effects = _d2.default.dispatch.apply(this, _d2.default.keys(__)).on('composite', function (d) {
    ...
    __.dimensions = pc.applyDimensionDefaults(_d2.default.keys(d.value));
    ...
    if (!_d2.default.keys(__.dimensions).length) pc.detectDimensions();
    pc.autoscale();
    if (typeof d.value === 'number') {
      if (d.value < _d2.default.keys(__.dimensions).length) {
  ...
  // expose events
  _d2.default.rebind(pc, events, 'on');

  // getter/setter with event firing
  function getset(obj, state, events) {
    _d2.default.keys(state).forEach(function (key) {
      obj[key] = function (x) {
        if (!arguments.length) {
          return state[key];
        }
  ...
  pc.autoscale = function () {
    ...
    var defaultScales = {
      'date': function date(k) {
        var extent = _d2.default.extent(__.data, function (d) {
          return d[k] ? d[k].getTime() : null;
        });

        // special case if single value
        if (extent[0] === extent[1]) {
          return _d2.default.scale.ordinal().domain([extent[0]]).rangePoints(getRange());
        }

        return _d2.default.time.scale().domain(extent).range(getRange());
      },
      'number': function number(k) {
        ...
        return _d2.default.scale.linear().domain(extent).range(getRange());
      },
      'string': function string(k) {
        ...
        domain = (0, _getOwnPropertyNames2.default)(counts).sort(function (a, b) {
          return counts[a] - counts[b];
        });

        return _d2.default.scale.ordinal().domain(domain).rangePoints(getRange());
      }
    };

    _d2.default.keys(__.dimensions).forEach(function (k) {
  ...
  pc.commonScale = function (global, type) {
    ...
    if (!_d2.default.keys(__.dimensions).length) {
      pc.detectDimensions();
    }
    pc.autoscale();

    // scales of the same type
    var scales = _d2.default.keys(__.dimensions).filter(function (p) {
      return __.dimensions[p].type == t;
    });

    if (global) {
      var extent = _d2.default.extent(scales.map(function (d, i) {
      ...
      scales.forEach(function (d) {
        __.dimensions[d].yscale.domain(_d2.default.extent(__.data, function (d) {
          return +d[k];
        }));
      });
    }
  ...
  pc.applyDimensionDefaults = function (dims) {
    ...
    dims = dims ? dims : _d2.default.keys(types);
    var newDims = {};
    var currIndex = 0;
  ...
  pc.getOrderedDimensionKeys = function () {
    return _d2.default.keys(__.dimensions).sort(function (x, y) {
      return _d2.default.ascending(__.dimensions[x].index, __.dimensions[y].index);
    });
  };
  ...
  pc.detectDimensionTypes = function (data) {
    var types = {};
    _d2.default.keys(data[0]).forEach(function (col) {
  ...
  var foregroundQueue = _d2.default.renderQueue(path_foreground).rate(50).clear(function () {
    pc.clear('foreground');
    pc.clear('highlight');
  });
  ...
  var brushedQueue = _d2.default.renderQueue(path_brushed).rate(50).clear(function () {
    pc.clear('brushed');
  });
  ...
  function compute_cluster_centroids(d) {
    var clusterCentroids = _d2.default.map();
    var clusterCounts = _d2.default.map();
    ...
    __.data.forEach(function (row) {
      _d2.default.keys(__.dimensions).map(function (p, i) {
        var scaled = __.dimensions[d].yscale(row[d]);
        if (!clusterCentroids.has(scaled)) {
          var map = _d2.default.map();
  ...
  function compute_centroids(row) {
    ...
    var p = _d2.default.keys(__.dimensions);
    var cols = p.length;
  ...
  pc.axisDots = function (r) {
    ...
    ctx.globalAlpha = _d2.default.min([1 / Math.pow(__.data.length, 1 / 2), 1]);
    __.data.forEach(function (d) {
      _d2.default.entries(__.dimensions).forEach(function (p, i) {
        ctx.beginPath();
  ...
  function path_brushed(d, i) {
    if (__.brushedColor !== null) {
      ctx.brushed.strokeStyle = _d2.default.functor(__.brushedColor)(d, i);
    } else {
      ctx.brushed.strokeStyle = _d2.default.functor(__.color)(d, i);
    }
    return color_path(d, ctx.brushed);
  };

  function path_foreground(d, i) {
    ctx.foreground.strokeStyle = _d2.default.functor(__.color)(d, i);
    return color_path(d, ctx.foreground);
  };

  function path_highlight(d, i) {
    ctx.highlight.strokeStyle = _d2.default.functor(__.color)(d, i);
  ...
  _d2.default.rebind(pc, axis, 'ticks', 'orient', 'tickValues', 'tickSubdivide', 'tickSize', 'tickPadding', 'tickFormat');
  ...
  function rotateLabels() {
    ...
    var delta = _d2.default.event.deltaY;
  ...
  pc.createAxes = function () {
    ...
    g.append('svg:g').attr('class', 'axis').attr('transform', 'translate(0,0)').each(function (d) {
      var axisElement = _d2.default.select(this).call(pc.applyAxisConfig(axis, __.dimensions[d]));
  ...
  pc.updateAxes = function (animationTime) {
    ...
    g_data.select('.axis').transition().duration(animationTime).each(function (d) {
      _d2.default.select(this).call(pc.applyAxisConfig(axis, __.dimensions[d]));
    });
  ...
  pc.applyAxisConfig = function (axis, dimension) {
    return axis.scale(dimension.yscale).orient(dimension.orient).ticks(dimension.ticks).tickValues(dimension.tickValues).innerTickSize(dimension.innerTickSize).outerTickSize(dimension.outerTickSize).tickPadding(dimension.tickPadding).tickFormat(dimension.tickFormat);
  };
  ...
  pc.reorderable = function () {
    ...
    g.style('cursor', 'move').call(_d2.default.behavior.drag().on('dragstart', function (d) {
      dragging[d] = this.__origin__ = xscale(d);
    }).on('drag', function (d) {
      dragging[d] = Math.min(w(), Math.max(0, this.__origin__ += _d2.default.event.dx));
      pc.sortDimensions();
      xscale.domain(pc.getOrderedDimensionKeys());
      pc.render();
      ...
      delete dragging[d];
      _d2.default.select(this).transition().attr('transform', 'translate(' + xscale(d) + ')');
      pc.render();
    }));
  ...
  pc.sortDimensionsByRowData = function (rowdata) {
    ...
    var positionSortedKeys = _d2.default.keys(__.dimensions).sort(function (a, b) {
  ...
  pc.brushModes = function () {
    return (0, _getOwnPropertyNames2.default)(brush.modes);
  };
  ...
  (function () {
    ...
    // data within extents
    function selected() {
      var actives = _d2.default.keys(__.dimensions).filter(is_brushed),
    ...
    function brushExtents(extents) {
      if (typeof extents === 'undefined') {
        var extents = {};
        _d2.default.keys(__.dimensions).forEach(function (d) {
          var brush = brushes[d];
          if (brush !== undefined && !brush.empty()) {
            var extent = brush.extent();
            extent.sort(_d2.default.ascending);
            extents[d] = extent;
          }
        });
        return extents;
        ...
        g.selectAll('.brush').each(function (d) {
          brushSelections[d] = _d2.default.select(this);
        });
    ...
    function brushFor(axis) {
      ...
      brush.y(__.dimensions[axis].yscale).on('brushstart', function () {
        if (_d2.default.event.sourceEvent !== null) {
    ...
    function brushReset(dimension) {
      if (dimension === undefined) {
        ...
        if (g) {
          g.selectAll('.brush').each(function (d) {
            _d2.default.select(this).transition().duration(0).call(brushes[d].clear());
          });
          pc.renderBrushed();
        }
      } else {
        if (g) {
          g.selectAll('.brush').each(function (d) {
            ...
            _d2.default.select(this).transition().duration(0).call(brushes[d].clear());
            brushes[d].event(_d2.default.select(this));
          });
          pc.renderBrushed();
        }
      }
    ...
    function install() {
      ...
      var brush = g.append('svg:g').attr('class', 'brush').each(function (d) {
        _d2.default.select(this).call(brushFor(d));
      });
  ...
  (function () {
    ...
    function drawStrum(strum, activePoint) {
      var svg = pc.selection.select('svg').select('g#strums'),
          ...
          drag = _d2.default.behavior.drag();
      ...
      circles.attr('cx', function (d) {
        ...
        _d2.default.select(this).style('opacity', 0.8);
      }).on('mouseout', function () {
        _d2.default.select(this).style('opacity', 0);
      }).call(drag);
    }

    function dimensionsForPoint(p) {
      var dims = { i: -1, left: undefined, right: undefined };
      _d2.default.keys(__.dimensions).some(function (dim, i) {
        if (xscale(dim) < p[0]) {
          var next = _d2.default.keys(__.dimensions)[pc.getOrderedDimensionKeys().indexOf(dim) + 1];
          dims.i = i;
          dims.left = dim;
          dims.right = next;
          return false;
      ...
      if (dims.left === undefined) {
        ...
        dims.i = _d2.default.keys(__.dimensions).length - 1;
        dims.right = dims.left;
        dims.left = pc.getOrderedDimensionKeys()[_d2.default.keys(__.dimensions).length - 2];
      }
    ...
    function onDragStart() {
      ...
      // should be allowed.
      return function () {
        var p = _d2.default.mouse(strumRect[0][0]),
            dims,
            strum;
    ...
    function selected() {
      var ids = (0, _getOwnPropertyNames2.default)(strums),
          brushed = __.data;
    ...
    function brushReset(strums) {
      return function () {
        var ids = (0, _getOwnPropertyNames2.default)(strums).filter(function (d) {
          return !isNaN(d);
        });
    ...
    function install() {
      ...
      pc.on('axesreorder.strums', function () {
        ...
        function consecutive(first, second) {
          var length = _d2.default.keys(__.dimensions).length;
          return _d2.default.keys(__.dimensions).some(function (d, i) {
  ...
  (function () {
    if (typeof _d2.default.svg.multibrush !== 'function') {
      return;
    }
    var brushes = {};
    ...
    function brushFor(axis) {
      var brush = _d2.default.svg.multibrush();
      ...
      brush.y(__.dimensions[axis].yscale).on('brushstart', function () {
        ...
        selection.style('visibility', null).attr('x', -15).attr('width', 30).style('fill', 'rgba(255,255,255,0.25)').style('stroke', 'rgba(0,0,0,0.6)');
      }).resizeAdaption(function (selection) {
        selection.selectAll('rect').attr('x', -15).attr('width', 30).style('visibility', null).style('fill', 'rgba(0,0,0,0.1)');
      });
    ...
    function brushReset(dimension) {
      ...
      if (g) {
        g.selectAll('.brush').each(function (d) {
          _d2.default.select(this).call(brushes[d].clear());
        });
        pc.renderBrushed();
      }
      return this;
  ...
  (function () {
    ...
    function onDragStart() {
      ...
      return function () {
        ...
        dims = dimensionsForPoint(p), arc = {
          ...
          endAngle: undefined,
          arc: _d2.default.svg.arc().innerRadius(0)
        };
    ...
    function selected() {
      var ids = (0, _getOwnPropertyNames2.default)(arcs),
          brushed = __.data;
    ...
    function brushReset(arcs) {
      return function () {
        var ids = (0, _getOwnPropertyNames2.default)(arcs).filter(function (d) {
          return !isNaN(d);
        });
  ...
  pc.highlight = function (data) {
    ...
    __.highlighted = data;
    pc.clear('highlight');
    _d2.default.selectAll([canvas.foreground, canvas.brushed]).classed('faded', true);
  ...
  pc.unhighlight = function () {
    __.highlighted = [];
    pc.clear('highlight');
    _d2.default.selectAll([canvas.foreground, canvas.brushed]).classed('faded', false);
    return this;
  };
  ...
  pc.toString = function () {
    return 'Parallel Coordinates: ' + _d2.default.keys(__.dimensions).length + ' dimensions (' + _d2.default.keys(__.data[0]).length + ' total) , ' + __.data.length + ' rows';
  };

  return pc;
};

_d2.default.renderQueue = function (func) {
  var _queue = [],
      // data to be rendered
  _rate = 10,
